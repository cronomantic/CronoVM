...
contiguous block of memory; whatever `malloc`/`free` semantics a program wants
...
reference allocator does for programs that don't want to roll their own.
...
| Host-side via syscall | In the host C program | Each `malloc` is a host call; allocator policy is fixed by the host, not the program |
...
allocator the program compiled in, with no boundary crossings.
...
A program's startup code can call them once to initialise its allocator and
...
```sh
cvm-translate user.bc -o program.bin --heap-reserve=4M
```
...
host program. Common patterns for sharing data across the boundary:

| Pattern | Direction | Example use |
| ------- | --------- | ----------- |
| Output buffer | `w` (VM writes, host reads) | Results the host consumes after a step — e.g. pixels, audio samples, a serialised message. |
| Command queue | `w` (VM writes, host reads) | A list of requests the host drains (I/O operations, RPCs, deferred jobs). |
| Input snapshot | `r` (host writes, VM reads) | State the host publishes each tick — input, sensor readings, configuration. |
| Streaming buffer | `w` (VM writes, host reads) | Bulk data produced incrementally. |
| Shared state | `rw` (both) | Anything where both sides update in place. |
...
Programs that don't want to bring their own allocator can drop in the
reference one. It's a header-only library — `#include "cvm_alloc.h"`
and the helpers compile into the user binary:

```sh
clang -emit-llvm -O1 -I runtime/lib -c program.c -o program.bc
cvm-translate program.bc -o program.bin --heap-reserve=4M
```
...
safety (single-threaded by design). For a program that needs better
...
A program can also just `#define malloc cvm_malloc` in a wrapper header so
that ported C code that uses `malloc`/`free` works unchanged.

## Patterns the project endorses

For program code, prefer in this order:
...
Avoid per-iteration `malloc` from the general-purpose allocator in hot loops.
The cost is small but it's not zero, and predictable latency matters more than
convenience here.
