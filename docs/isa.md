...
| 0x3A | `SETJMP` | `A, B` | C `setjmp`. `B` holds the `jmp_buf` heap address; saves `{resume pc, SP, dest reg A}` into the buf (3×u32) and sets `R[A]=0`, then falls through. The register file and `pc` are interpreter locals and the call stack lives in the heap (`CALL` pushes only the return PC), so the buf need not capture registers — the calling convention has already spilled the setjmp-caller's live values to its frame. Surfaced by the translator from a `setjmp` call (result in `R[A]`). |
| 0x3B | `LONGJMP` | `A, B` | C `longjmp`. `A` holds the `jmp_buf` address, `B` the value; reloads `{pc, SP, dest}` from the buf, sets `R[dest] = R[B]` (mapped to 1 if 0, per C), restores `SP`, and jumps to `pc` — resuming right after the matching `SETJMP` with the value in `R[dest]`, where the caller reloads its spilled locals from the restored frame. Traps `CVM_E_BAD_PC`/`CVM_E_BAD_ADDR` on a corrupt buf. Surfaced from a `longjmp` call. |
| 0x3C | `CORO_SWAP` | `A, B` | Cooperative coroutine context swap. `R[A]` holds `&from`, `R[B]` holds `&to`. Operates on 16-byte context records `{u32 pc, u32 sp, u32 dest, u32 status}` — the first 3 words match `jmp_buf`, the 4th adds `status` (`CORO_FRESH=0`, `CORO_RUNNING=1`, `CORO_SUSPENDED=2`, `CORO_DEAD=3`). Writes `from = {next pc, current SP, A, SUSPENDED}`. Reads `to`; rejects `to.status` ∉ {FRESH, SUSPENDED} with `CVM_E_BAD_CORO_STATE`. If `to.status == FRESH`, treats `to.pc` as a function index and does `to.pc = FUNCS[to.pc]` (the libco-style "first-run" launch via the trampoline); else uses `to.pc` directly. Writes `to.status = RUNNING`. On FRESH resume, sets `R[to.dest] = (int32_t)to` (passes the new coroutine pointer to its entry function as arg0 — `to.dest` is typically 0 = the ABI's arg0 register). On SUSPENDED resume, performs NO register write (the resumed program's caller-saved regs survive the swap intact, no `returns_twice` needed at the call site; "who resumed me" is recoverable from the program-managed `to->resumer` field). Restores `SP = to.sp`, jumps to `to.pc`. Bounds-checked: `from`/`to` must each have 16 bytes; `from == to` is rejected as a programming bug (`CVM_E_BAD_ADDR`); the lookup-up PC is range-checked against `code_count`; `to.dest` against `CVM_REG_COUNT`. Surfaced from `__cvm_coro_swap_raw(from, to)` (the program-side `cron_coro_swap` wraps this to also set `to->resumer = from` and similar bookkeeping; see `coro.h`). |
| 0x3D | `FFLOOR` | `A, B` | `R[A] = floorf(f32(R[B]))` — single-precision round toward −∞ via the host's `floorf()`. NaN/±Inf propagate; values with `\|x\| ≥ 2²³` are already integral and pass through. Surfaced by the translator from `llvm.floor.f32`. |
| 0x3E | `FCEIL` | `A, B` | `R[A] = ceilf(f32(R[B]))` — single-precision round toward +∞ via the host's `ceilf()`. Same NaN/Inf/large-value behaviour as `FFLOOR`. Surfaced from `llvm.ceil.f32` (e.g. libc++'s `std::ceil` in the `unordered_map` rehash load-factor). |
| 0x3F | `FTRUNC` | `A, B` | `R[A] = truncf(f32(R[B]))` — single-precision round toward zero via the host's `truncf()`. Same NaN/Inf/large-value behaviour as `FFLOOR`. Surfaced from `llvm.trunc.f32`. |
