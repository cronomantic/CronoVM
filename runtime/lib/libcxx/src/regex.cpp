...
// [CRONOPIO] picolibc's ctype_base::mask is a SIGNED `char`, so masks with the
// high bit set (blank=0x80, print=0x97) are negative and ill-formed-narrow into
// char_class_type (unsigned short) in this aggregate init. Cast through unsigned
// char to preserve the ctype bit pattern (low byte) — the only local edit to the
// vendored regex.cpp (see libcxx/VENDOR.md). __regex_word is already
// char_class_type (0x8000) and must NOT pass through unsigned char.
#define _CRON_CC(x) static_cast<regex_traits<char>::char_class_type>(static_cast<unsigned char>(x))
const classnames ClassNames[] = {
    {"alnum", _CRON_CC(ctype_base::alnum)},
    {"alpha", _CRON_CC(ctype_base::alpha)},
    {"blank", _CRON_CC(ctype_base::blank)},
    {"cntrl", _CRON_CC(ctype_base::cntrl)},
    {"d", _CRON_CC(ctype_base::digit)},
    {"digit", _CRON_CC(ctype_base::digit)},
    {"graph", _CRON_CC(ctype_base::graph)},
    {"lower", _CRON_CC(ctype_base::lower)},
    {"print", _CRON_CC(ctype_base::print)},
    {"punct", _CRON_CC(ctype_base::punct)},
    {"s", _CRON_CC(ctype_base::space)},
    {"space", _CRON_CC(ctype_base::space)},
    {"upper", _CRON_CC(ctype_base::upper)},
    {"w", regex_traits<char>::__regex_word},
    {"xdigit", _CRON_CC(ctype_base::xdigit)}};
#undef _CRON_CC
